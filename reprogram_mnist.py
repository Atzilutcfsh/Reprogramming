...
reprogrammer.eval()
os.makedirs("reprogram_viz", exist_ok=True)
...
for i in range(10):
    ...
    vutils.save_image(img, f"reprogram_viz/img_{i}_label_{labels[i].item()}.png")

print("✅ 已儲存 10 張 reprogrammed 圖像至資料夾：reprogram_viz/")
...
plt.savefig("reprogram_viz/reprogrammed_grid.png")
plt.close()
...
os.makedirs("reprogram_viz/mask", exist_ok=True)
for c in range(3):
    vutils.save_image(mask[c].unsqueeze(0).clamp(0, 1), f"reprogram_viz/mask/mask_channel_{c}.png")

# 2️⃣ 將 RGB mask 組合並存為一張彩圖
rgb_mask = mask.clone()
rgb_mask = rgb_mask.clamp(0, 1)  # 限制值域到 [0, 1]
vutils.save_image(rgb_mask, "reprogram_viz/mask/mask_rgb.png")

# 3️⃣ 將 mask tensor 存成 .pt 檔案
torch.save(mask, "reprogram_viz/mask/mask_tensor.pt")
